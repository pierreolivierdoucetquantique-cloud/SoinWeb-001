...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function startBooking(formula) {
    ...
    // Toujours relire le prix en temps réel depuis PO_Pricing (Admin → Tarifs)
  ...
  document.getElementById('cta-button')?.addEventListener('click', (e) => {
    ...
    if (PO_Auth.getCurrentUser()) {
      e.preventDefault();
      // Le bouton principal de la page ne cible aucune formule précise —
      // on utilise donc la première formule disponible pour ce service
      // (source unique de vérité : Admin → Tarifs) plutôt que de laisser
      // le prix à 0 (c'était la cause du bug "0,00 $ CAD" affiché au
      // paiement lorsque la réservation partait de ce bouton).
  ...
  formulasContainer?.addEventListener('click', (e) => {
    ...
    if (!btn) return;
    // Relire le prix depuis PO_Pricing pour garantir la synchro temps réel avec Admin → Tarifs
    const fId = btn.dataset.formulaId || '';
    // Relire le prix depuis PO_Pricing pour garantir la synchro temps réel

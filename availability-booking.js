...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function confirmSlot(btn) {
    ...
    const result = await PO_Auth.createAppointment({
    ...
    if (!result.ok) {
      showNotice(result.error || 'Ce créneau vient d\'être réservé. Veuillez en choisir un autre.', 'error');
      document.querySelectorAll('.cal-slot-btn').forEach(b => { b.disabled = false; });
      btn.classList.remove('cal-slot-btn--confirming');

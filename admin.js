...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('service-page-form').addEventListener('submit', (e) => {
    ...
    notice.hidden = false;
    notice.dataset.tone = 'success';
    notice.textContent = 'Page de service mise à jour. Les changements sont sauvegardés sur le serveur et visibles sur la page publique après rechargement.';
  ...
  function renderStepsList() {
    ...
    list.querySelectorAll('[data-step-field]').forEach(input => {
      input.addEventListener('change', () => {
        ...
        const value = field === 'type' ? input.value : input.value.trim();
        PO_Content.updateServiceStep(serviceId, idx, { [field]: value });
        if (field === 'type') renderStepsList();
      });
    });
    ...
    list.querySelectorAll('[data-step-remove]').forEach(btn => {
      btn.addEventListener('click', () => {
        askConfirm('Supprimer cette section ?', 'Elle disparaîtra de la page publique après rechargement.', () => {
  ...
  document.getElementById('cm-add-step').addEventListener('click', () => {
    const serviceId = document.getElementById('content-service-filter').value;
    PO_Content.addStep(serviceId, { title: 'Nouvelle section', type: 'text', text: '' });
    renderStepsList();
  });

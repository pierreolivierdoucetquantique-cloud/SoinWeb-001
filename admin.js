...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  /* ---------- INITIALISATION SESSION (backend réel) ---------- */
  await PO_Auth.init();
  ...
  /* ---------- INIT RÉSILIENTE : bouton "Enregistrer" — Présentation de
     Pierre-Olivier (page Mon histoire) ----------
     admin.js est un seul très long gestionnaire DOMContentLoaded : si
     N'IMPORTE QUELLE ligne plus bas dans ce fichier lève une erreur non
     interceptée (élément introuvable, etc.), tout le code qui suit —
     y compris le branchement du bouton Enregistrer de cette section,
     normalement situé bien plus loin — n'est jamais exécuté, et le
     bouton reste visible mais inerte. On sécurise donc son initialisation
     ici, tout en haut, dans son propre try/catch, indépendamment de tout
     le reste. Les fonctions renderBioCMS()/saveBioCMS() sont des
     déclarations de fonction (hoisted) définies plus bas dans ce même
     scope : elles restent appelables ici sans problème d'ordre. */
  try {
    const bioSaveBtn = document.getElementById('bio-save-btn');
    const bioAddBtn = document.getElementById('bio-add-section');
    const bioLoadNewTextBtn = document.getElementById('bio-load-new-text');
    const bioPhotoUploadBtn = document.getElementById('bio-photo-upload-btn');
    const bioPhotoFile = document.getElementById('bio-photo-file');
    const bioPhotoDeleteBtn = document.getElementById('bio-photo-delete-btn');

    bioSaveBtn?.addEventListener('click', () => saveBioCMS());
    bioAddBtn?.addEventListener('click', async () => {
      await PO_Content.addBioSection('text');
      renderBioCMS();
    });
    // Remplit le formulaire (titre, sous-titre, toutes les sections) avec le
    // nouveau texte "Je suis Pierre-Olivier" fourni pour la refonte de cette
    // page. N'écrit rien sur le serveur : seul un clic sur "Enregistrer
    // tout" ensuite persiste réellement le changement — Pierre-Olivier
    // garde la main pour relire avant d'envoyer.
    bioLoadNewTextBtn?.addEventListener('click', () => {
      askConfirm(
        'Charger le nouveau texte ?',
        'Le titre, le sous-titre et toutes les sections actuelles seront remplacés dans ce formulaire par le nouveau texte "Je suis Pierre-Olivier". Votre photo n\'est pas touchée. Rien n\'est enregistré tant que vous ne cliquez pas sur "Enregistrer tout".',
        () => {
          const fresh = PO_Content.getDefaultBio();
          const bio = PO_Content.getBio();
          bio.hero = fresh.hero;
          bio.sections = fresh.sections;
          renderBioCMS();
          const notice = document.getElementById('bio-cms-notice');
          if (notice) {
            notice.hidden = false;
            notice.dataset.tone = 'success';
            notice.textContent = 'Nouveau texte chargé dans le formulaire. Relisez, puis cliquez sur "Enregistrer tout" pour le publier.';
            setTimeout(() => { notice.hidden = true; }, 5000);
          }
        }
      );
    });
    bioPhotoUploadBtn?.addEventListener('click', () => bioPhotoFile?.click());
    bioPhotoFile?.addEventListener('change', (e) => {
      ...
      PO_Photo.openCropper(file, {
        onSave: async (dataUrl) => {
          const res = await PO_Content.updateBioPhoto(dataUrl);
          renderBioCMS();
          showToast(res.ok ? 'Photo enregistrée sur le serveur.' : ('Échec : ' + (res.error || 'erreur inconnue.')), res.ok ? 'success' : 'error');
        }
      });
    });
    bioPhotoDeleteBtn?.addEventListener('click', () => {
      askConfirm('Supprimer la photo de la page biographie ?', '', async () => {
        await PO_Content.updateBioPhoto('');
        renderBioCMS();
      });
    });

    if (bioSaveBtn) renderBioCMS();
  } catch (err) {
    console.warn('[Bio CMS] Initialisation échouée :', err.message);
  }
  ...
  function showPanel(panelId) {
    ...
    if (panelId === 'site-settings') { renderSiteSettings(); renderIntegrationsStatus(); }
    if (panelId === 'buttons') renderButtonSettings();
  ...
  document.getElementById('admin-logout-link').addEventListener('click', (e) => {
    e.preventDefault();
    PO_Auth.logout().then(() => { window.location.href = 'index.html'; });
  });
    ...
    tbody.querySelectorAll('[data-delete-client]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        askConfirm(
          ...
          () => { PO_Auth.deleteClientAsAdmin(client.id).then(() => renderClients()); }
        );
      });
    });
  }
  ...
  document.getElementById('drawer-unblock-client')?.addEventListener('click', () => {
    ...
    askConfirm(
      ...
      () => {
        PO_Auth.unblockClient(_drawerClientId).then(() => {
  ...
  document.getElementById('drawer-delete-client').addEventListener('click', () => {
    ...
    askConfirm(
      ...
      `${client.firstName} ${client.lastName} et tous ses rendez-vous seront définitivement supprimés.`,
      () => {
        PO_Auth.deleteClientAsAdmin(client.id).then(() => {
  ...
  function renderMonthView() {
    ...
    dayCells.forEach((cell, index) => {
      ...
      cell.addEventListener('drop', async (e) => {
        ...
        if (!appt) return;
        const result = await PO_Auth.moveAppointment(draggedApptId, targetDate, appt.time);
        if (!result.ok) {
  ...
  function _wireTimelineInteractions(container) {
    ...
    container.querySelectorAll('[data-tl-col]').forEach(col => {
      ...
      col.addEventListener('drop', async (e) => {
        ...
        const result = await PO_Auth.moveAppointment(draggedApptId, targetDate, newTime);
        if (!result.ok) {
          showToast(result.error || 'Impossible de déplacer ce rendez-vous.', 'error');
        } else {
  ...
  function quickSetAppointmentStatus(apptId, newStatus) {
    ...
    PO_Auth.updateAppointment(apptId, { status: newStatus }).then(result => {
      if (!result.ok) {
    ...
    list.querySelectorAll('[data-delete-appt]').forEach(btn => {
      btn.addEventListener('click', () => {
        askConfirm('Supprimer ce rendez-vous ?', 'Cette action est irréversible.', () => {
          PO_Auth.deleteAppointment(btn.dataset.deleteAppt).then(() => renderCalendar());
        });
      });
    });
    list.querySelectorAll('[data-delete-series]').forEach(btn => {
      btn.addEventListener('click', () => {
        askConfirm('Supprimer toute la série récurrente ?', 'Toutes les occurrences liées à cette série seront supprimées. Cette action est irréversible.', () => {
          PO_Auth.deleteAppointmentSeries(btn.dataset.deleteSeries).then(() => renderCalendar());
        });
      });
    });
  }
  ...
  apptForm.addEventListener('submit', async (e) => {
    ...
    if (isRecurring) {
      ...
      const result = await PO_Auth.createRecurringAppointments({ ...data, frequency, occurrences });
    ...
    if (editingId) {
      ...
      saveResult = await PO_Auth.updateAppointment(editingId, data);
    } else {
      saveResult = await PO_Auth.createAppointment(data);
    }

    if (!saveResult.ok) {
      apptNotice.hidden = false;
      apptNotice.dataset.tone = 'error';
      apptNotice.textContent = saveResult.error || "Ce créneau n'est plus disponible.";
      return;
    }
  ...
  vacationForm.addEventListener('submit', async (e) => {
    ...
    await PO_Auth.addVacation({ startDate, endDate, label: document.getElementById('vacation-label').value });
  ...
  async function enterAvailabilityDraft() {
    const av = await PO_Auth.getAvailability();
  ...
  async function renderAvailabilityPanel() {
    if (!draftAvailability) await enterAvailabilityDraft();
  ...
  document.getElementById('avail-draft-save').addEventListener('click', async () => {
    const result = await PO_Auth.replaceAvailability(draftAvailability);
    if (result.ok) {
      await enterAvailabilityDraft();
      refreshDraftBar();
      renderCalendar();
  ...
  document.getElementById('avail-draft-cancel').addEventListener('click', async () => {
  ...
  document.getElementById('threshold-content-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const notice = document.getElementById('threshold-content-notice');
    const btn = e.target.querySelector('button[type="submit"]');
    if (btn) btn.disabled = true;
    const res = await PO_Content.updateThreshold({
    ...
    notice.hidden = false;
    notice.dataset.tone = res.ok ? 'success' : 'error';
    notice.textContent = res.ok ? 'Modal "Lieu Sacré" enregistré sur le serveur.' : ('Échec de l\'enregistrement : ' + (res.error || 'erreur inconnue.'));
    setTimeout(() => { notice.hidden = true; }, 3000);
  });

  document.getElementById('brand-content-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const notice = document.getElementById('brand-content-notice');
    const btn = e.target.querySelector('button[type="submit"]');
    if (btn) btn.disabled = true;
    const res = await PO_Content.updateBrand({
      homeTagline: document.getElementById('cm-brand-tagline').value.trim()
    });
    if (btn) btn.disabled = false;
    notice.hidden = false;
    notice.dataset.tone = res.ok ? 'success' : 'error';
    notice.textContent = res.ok ? 'Texte de marque enregistré sur le serveur.' : ('Échec de l\'enregistrement : ' + (res.error || 'erreur inconnue.'));
    setTimeout(() => { notice.hidden = true; }, 3000);
  });

  document.getElementById('shared-labels-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const notice = document.getElementById('shared-labels-notice');
    const btn = e.target.querySelector('button[type="submit"]');
    if (btn) btn.disabled = true;
    const res = await PO_Content.updateSharedLabels({
    ...
    notice.hidden = false;
    notice.dataset.tone = res.ok ? 'success' : 'error';
    notice.textContent = res.ok ? 'Libellés partagés enregistrés sur les 3 pages de service.' : ('Échec de l\'enregistrement : ' + (res.error || 'erreur inconnue.'));
    setTimeout(() => { notice.hidden = true; }, 3000);
  });
  ...
  document.getElementById('service-page-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const serviceId = document.getElementById('content-service-filter').value;
    const btn = e.target.querySelector('button[type="submit"]');
    if (btn) btn.disabled = true;

    const res = await PO_Content.updateServicePageContent(serviceId, {
    ...
    notice.textContent = res.ok
      ? 'Page de service enregistrée sur le serveur. Visible sur la page publique après rechargement.'
      : ('Échec de l\'enregistrement : ' + (res.error || 'erreur inconnue. Réessayez.'));
    setTimeout(() => { notice.hidden = true; }, 3500);
  });

  /* ---- Sections de la page (texte libre ou liste) — liste dynamique ----
     Aucune sauvegarde automatique au clavier : les champs de texte ne sont
     envoyés au serveur qu'au clic sur "Enregistrer les sections", avec une
     confirmation claire de succès ou d'échec. Seuls l'ajout, la suppression
     et le réordonnancement (actions structurelles, pas du texte) restent
     immédiats — comme pour la présentation de Pierre-Olivier. */
  function renderStepsList() {
    ...
    if (!steps.length) {
      list.innerHTML = '<p class="cm-block-empty">Aucune section pour le moment. Utilisez "+ Ajouter une section".</p>';
      return;
    }

    list.innerHTML = steps.map((s, i) => `
      <div class="cm-block-item" data-step-row="${i}" style="flex-direction:column; align-items:stretch;">
        <div style="display:flex; gap:10px; align-items:center;">
          ...
          <select data-step-field="type" data-step-idx="${i}" style="max-width:160px;">
            <option value="text" ${(s.type || 'text') === 'text' ? 'selected' : ''}>Paragraphes</option>
            <option value="list" ${s.type === 'list' ? 'selected' : ''}>Liste à puces</option>
          </select>
          <button type="button" class="cm-block-item__remove" data-step-remove="${i}" title="Supprimer cette section" style="margin-left:auto;">✕</button>
        </div>
        <div class="cm-block-item__fields" style="margin-top:10px;">
          <input type="text" data-step-field="eyebrow" data-step-idx="${i}" value="${escapeHtml(s.eyebrow || '')}" placeholder="Sur-titre (facultatif)">
          <input type="text" data-step-field="title" data-step-idx="${i}" value="${escapeHtml(s.title)}" placeholder="Titre de la section">
          <textarea rows="2" data-step-field="intro" data-step-idx="${i}" placeholder="Phrase d'introduction (facultatif)">${escapeHtml(s.intro || '')}</textarea>
          <textarea rows="5" data-step-field="text" data-step-idx="${i}" placeholder="${(s.type === 'list') ? 'Un élément de liste par ligne' : 'Le texte — laissez une ligne vide entre les paragraphes'}">${escapeHtml(s.text)}</textarea>
          <textarea rows="2" data-step-field="outro" data-step-idx="${i}" placeholder="Phrase de conclusion, en italique (facultatif)">${escapeHtml(s.outro || '')}</textarea>
        </div>
      </div>
    `).join('') + `
      <div style="display:flex; align-items:center; gap:14px; margin-top:4px;">
        <button type="button" class="btn btn--gold btn--sm" id="cm-steps-save-btn">Enregistrer les sections</button>
        <span class="auth__notice" id="cm-steps-notice" hidden></span>
      </div>
    `;

    // Changer le type met juste à jour le texte d'aide (placeholder) de cette
    // ligne, sans ré-afficher toute la liste — pour ne jamais perdre une
    // modification non enregistrée dans une autre section.
    list.querySelectorAll('[data-step-field="type"]').forEach(select => {
      select.addEventListener('change', () => {
        const idx = Number(select.dataset.stepIdx);
        const row = list.querySelector(`[data-step-row="${idx}"]`);
        const textArea = row && row.querySelector('[data-step-field="text"]');
        if (textArea) {
          textArea.placeholder = select.value === 'list' ? 'Un élément de liste par ligne' : 'Le texte — laissez une ligne vide entre les paragraphes';
        }
      });
    });
    list.querySelectorAll('[data-step-up]').forEach(btn => {
      btn.addEventListener('click', async () => {
        await PO_Content.moveStep(serviceId, Number(btn.dataset.stepUp), -1);
        renderStepsList();
      });
    });
    list.querySelectorAll('[data-step-down]').forEach(btn => {
      btn.addEventListener('click', async () => {
        await PO_Content.moveStep(serviceId, Number(btn.dataset.stepDown), 1);
        renderStepsList();
      });
    });
    list.querySelectorAll('[data-step-remove]').forEach(btn => {
      btn.addEventListener('click', () => {
        askConfirm('Supprimer cette section ?', 'Elle disparaîtra de la page publique après enregistrement.', async () => {
          await PO_Content.removeStep(serviceId, Number(btn.dataset.stepRemove));
          renderStepsList();
        });
      });
    });

    document.getElementById('cm-steps-save-btn').addEventListener('click', async () => {
      const notice = document.getElementById('cm-steps-notice');
      const saveBtn = document.getElementById('cm-steps-save-btn');
      saveBtn.disabled = true;
      notice.hidden = false;
      notice.dataset.tone = 'success';
      notice.textContent = 'Enregistrement…';

      const rows = Array.from(list.querySelectorAll('[data-step-row]'));
      const results = await Promise.all(rows.map(row => {
        const idx = Number(row.dataset.stepRow);
        const patch = {};
        row.querySelectorAll('[data-step-field]').forEach(field => {
          const key = field.dataset.stepField;
          patch[key] = key === 'type' ? field.value : field.value.trim();
        });
        return PO_Content.updateServiceStep(serviceId, idx, patch);
      }));

      saveBtn.disabled = false;
      const failed = results.filter(r => !r.ok);
      notice.hidden = false;
      notice.dataset.tone = failed.length ? 'error' : 'success';
      notice.textContent = failed.length
        ? `Échec de l'enregistrement de ${failed.length} section(s). Vérifiez votre connexion et réessayez.`
        : 'Sections enregistrées sur le serveur et visibles sur la page publique après rechargement.';
      setTimeout(() => { notice.hidden = true; }, 3500);
    });
  }

  document.getElementById('cm-add-step').addEventListener('click', async () => {
    const serviceId = document.getElementById('content-service-filter').value;
    await PO_Content.addStep(serviceId, { title: 'Nouvelle section', type: 'text', text: '' });
    renderStepsList();
  });

  /* ---- FAQ — liste dynamique (même principe : bouton Enregistrer explicite) ---- */
  function renderFaqList() {
    ...
    `).join('') + `
      <div style="display:flex; align-items:center; gap:14px; margin-top:4px;">
        <button type="button" class="btn btn--gold btn--sm" id="cm-faq-save-btn">Enregistrer la FAQ</button>
        <span class="auth__notice" id="cm-faq-notice" hidden></span>
      </div>
    `;

    list.querySelectorAll('[data-faq-up]').forEach(btn => {
      btn.addEventListener('click', async () => {
        await PO_Content.moveFaqItem(serviceId, Number(btn.dataset.faqUp), -1);
        renderFaqList();
      });
    });
    list.querySelectorAll('[data-faq-down]').forEach(btn => {
      btn.addEventListener('click', async () => {
        await PO_Content.moveFaqItem(serviceId, Number(btn.dataset.faqDown), 1);
        renderFaqList();
      });
    });
    list.querySelectorAll('[data-faqitem-remove]').forEach(btn => {
      btn.addEventListener('click', () => {
        askConfirm('Supprimer cette question ?', 'Elle disparaîtra de la page publique après enregistrement.', async () => {
          await PO_Content.removeFaqItem(serviceId, Number(btn.dataset.faqitemRemove));
          renderFaqList();
        });
      });
    });

    document.getElementById('cm-faq-save-btn').addEventListener('click', async () => {
      const notice = document.getElementById('cm-faq-notice');
      const saveBtn = document.getElementById('cm-faq-save-btn');
      saveBtn.disabled = true;
      notice.hidden = false;
      notice.dataset.tone = 'success';
      notice.textContent = 'Enregistrement…';

      const rows = Array.from(list.querySelectorAll('[data-faq-row]'));
      const results = await Promise.all(rows.map(row => {
        const idx = Number(row.dataset.faqRow);
        const patch = {};
        row.querySelectorAll('[data-faqitem-field]').forEach(field => {
          patch[field.dataset.faqitemField] = field.value.trim();
        });
        return PO_Content.updateServiceFaq(serviceId, idx, patch);
      }));
      ...
      notice.dataset.tone = failed.length ? 'error' : 'success';
      notice.textContent = failed.length
        ? `Échec de l'enregistrement de ${failed.length} question(s). Vérifiez votre connexion et réessayez.`
        : 'FAQ enregistrée sur le serveur et visible sur la page publique après rechargement.';
      setTimeout(() => { notice.hidden = true; }, 3500);
    });
  }

  document.getElementById('cm-add-faq').addEventListener('click', async () => {
    const serviceId = document.getElementById('content-service-filter').value;
  ...
  function renderBioCMS() {
    ...
    container.querySelectorAll('.bio-section-up').forEach(btn => {
      btn.addEventListener('click', async () => {
        await PO_Content.moveBioSection(btn.dataset.sid, -1);
        renderBioCMS();
      });
    });
    ...
    container.querySelectorAll('.bio-section-remove').forEach(btn => {
      btn.addEventListener('click', () => {
        askConfirm('Supprimer cette section ?', '', async () => {
          await PO_Content.removeBioSection(btn.dataset.sid);
          renderBioCMS();
        });
      });
    });
  ...
  async function saveBioCMS() {
    ...
    for (const el of document.querySelectorAll('[data-bio-section-id]')) {
      ...
      results.push(await PO_Content.updateBioSection(sid, data));
    }
    ...
    notice.dataset.tone = failed.length ? 'error' : 'success';
    notice.textContent = failed.length
      ? `Échec de l'enregistrement (${failed.length} élément(s)). Vérifiez votre connexion et réessayez.`
      : 'Modifications enregistrées sur le serveur. Visibles sur la page "Mon histoire" après rechargement.';
    setTimeout(() => { notice.hidden = true; }, 3500);
  }

  // Le branchement du bouton Enregistrer, de l'ajout de section et de la
  // photo, ainsi que l'appel initial à renderBioCMS(), sont désormais faits
  // tout en haut de ce fichier (voir "INIT RÉSILIENTE" juste après la garde
  // d'accès admin), pour que cette section reste fonctionnelle même si une
  // erreur survient ailleurs plus bas dans ce long fichier.
  ...
  function renderServices() {
    ...
    container.querySelectorAll('.service-edit-form').forEach(form => {
      form.addEventListener('submit', async (e) => {
        ...
        const submitBtn = form.querySelector('.service-edit-card__save');
        const note = form.querySelector('.svc-saved-note');
        submitBtn.disabled = true;
        const res = await PO_Content.updateService(serviceId, patch);
        submitBtn.disabled = false;
        note.style.color = res.ok ? '#5fd99c' : '#e8743b';
        note.textContent = res.ok ? 'Enregistré sur le serveur ✓' : ('Échec : ' + (res.error || 'erreur inconnue.'));
        setTimeout(() => { note.textContent = ''; }, res.ok ? 2000 : 4000);
        if (res.ok) {
    ...
    tbody.querySelectorAll('[data-delete-formula]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        askConfirm(
          ...
          () => { PO_Content.deleteFormula(formula.id).then(() => renderPricing()); }
        );
      });
    });
  }
  ...
  formulaForm.addEventListener('submit', async (e) => {
    ...
    await PO_Content.saveFormula(payload);
  ...
  const STRIPE_API_BASE_URL = 'https://ntabou-aka-we.onrender.com';
    ...
    tbody.querySelectorAll('[data-confirm-tx]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        askConfirm(
          ...
          async () => {
            const res = await PO_Auth.confirmTransaction(tx.id);
            if (res.ok && typeof PO_Notifications !== 'undefined') {
              const client = PO_Auth.listClients().find(c => c.id === tx.clientId);
              if (client) {
                // Le courriel de confirmation réel (enrichi : numéro de
                // confirmation, Zoom, Google Calendar) est désormais envoyé
                // directement par le serveur, dans la même requête que la
                // confirmation elle-même (voir confirmation-workflow.js) —
                // ce journal local reste utile pour l'historique visible
                // dans Admin → Notifications, sans déclencher un second envoi.
                PO_Notifications.logEmail({
                  type: 'appointment_confirmation',
                  to: client.email,
    ...
    tbody.querySelectorAll('[data-refuse-tx]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        askConfirm(
          ...
          async () => {
            const res = await PO_Auth.rejectTransaction(tx.id);
            if (res.ok && typeof PO_Notifications !== 'undefined') {
              const client = PO_Auth.listClients().find(c => c.id === tx.clientId);
              if (client) {
                PO_Notifications.logEmail({
    ...
    tbody.querySelectorAll('[data-cancel-tx]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        askConfirm(
          ...
          () => { PO_Auth.cancelTransaction(tx.id).then(() => { renderPayments(); renderCalendar(); }); }
        );
      });
    });
    ...
    tbody.querySelectorAll('[data-delete-tx]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        askConfirm(
          ...
          () => {
            PO_Auth.deleteTransaction(tx.id).then(() => {
  ...
  document.getElementById('pay-settings-save')?.addEventListener('click', async () => {
    await PO_Auth.updatePaymentSettings({
  ...
  document.getElementById('dashboard-purge-btn')?.addEventListener('click', () => {
    askConfirm(
      ...
      async () => {
        await PO_Auth.purgeAllTestData();
  ...
  /* =========================================================
     INTÉGRATIONS — Zoom, Google Calendar, Resend, Stripe
     (statut lecture seule + connexion Google Calendar en un clic)
  ========================================================= */
  async function renderIntegrationsStatus() {
    const listEl = document.getElementById('integrations-status-list');
    if (!listEl) return;
    listEl.innerHTML = '<p class="empty-state">Vérification…</p>';
    const res = await PO_Api.get('/api/admin/integrations-status');
    if (!res.ok) {
      listEl.innerHTML = `<p class="empty-state">Impossible de vérifier le statut des intégrations (${escapeHtml(res.error || 'erreur inconnue')}).</p>`;
      return;
    }
    const rows = [
      { label: 'Zoom (réunions automatiques)', ok: res.zoom.configured, hint: res.zoom.configured ? 'Configuré' : 'ZOOM_ACCOUNT_ID / ZOOM_CLIENT_ID / ZOOM_CLIENT_SECRET manquants' },
      { label: 'Google Calendar', ok: res.googleCalendar.configured && res.googleCalendar.authorized,
        hint: !res.googleCalendar.configured ? 'GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET / GOOGLE_REDIRECT_URI manquants'
          : (res.googleCalendar.authorized ? 'Connecté' : 'Configuré, mais pas encore autorisé — cliquez "Connecter Google Calendar"') },
      { label: 'Courriels (Resend)', ok: res.resend.configured, hint: res.resend.configured ? 'Configuré' : 'RESEND_API_KEY manquante' },
      { label: 'Paiement carte (Stripe)', ok: res.stripe.configured, hint: res.stripe.configured ? 'Configuré' : 'STRIPE_SECRET_KEY manquante' }
    ];
    listEl.innerHTML = rows.map(r => `
      <div class="notif-setting-row">
        <div>
          <div class="notif-setting-row__label">${r.ok ? '🟢' : '🔴'} ${escapeHtml(r.label)}</div>
          <div class="notif-setting-row__desc">${escapeHtml(r.hint)}</div>
        </div>
      </div>
    `).join('');
  }

  document.getElementById('integrations-refresh-btn')?.addEventListener('click', renderIntegrationsStatus);

  document.getElementById('google-connect-btn')?.addEventListener('click', async () => {
    const notice = document.getElementById('integrations-notice');
    notice.hidden = true;
    const res = await PO_Api.get('/api/admin/google/auth-url');
    if (!res.ok) {
      notice.hidden = false;
      notice.dataset.tone = 'error';
      notice.textContent = res.error || "Impossible de générer le lien d'autorisation Google.";
      return;
    }
    window.open(res.url, '_blank', 'noopener');
    showToast('Fenêtre Google ouverte — autorisez l\'accès puis revenez ici et cliquez "Actualiser le statut".', 'success');
  });

  /* =========================================================
     GESTION DES BOUTONS (page d'accueil)
  ========================================================= */
  const BUTTON_SETTING_LABELS = {
    connexion:      { label: 'Connexion',                desc: "Portail de connexion sur la page d'accueil (la page /connexion.html elle-même reste toujours accessible, pour éviter tout risque de blocage de l'accès admin)." },
    energetiques:   { label: 'Soins Énergétiques',        desc: 'Portail et page du service Soins Énergétiques.' },
    accompagnement: { label: 'Accompagnement 1:1',        desc: "Portail et page du service Accompagnement 1:1." },
    inscription:    { label: 'Créer un compte',           desc: "Portail d'inscription sur la page d'accueil et page /inscription.html." },
    histoire:       { label: 'Histoire de Pierre-Olivier', desc: 'Portail et page biographique.' },
    temoignages:    { label: 'Témoignages',               desc: 'Bouton révélant la section témoignages sur la page d\'accueil.' },
    contact:        { label: 'Contact',                   desc: 'Lien "Contact" affiché dans le pied de page de tout le site, et page /contact.html.' }
  };

  function renderButtonSettings() {
    const settings = PO_Content.getButtonSettings();
    const listEl = document.getElementById('button-settings-list');
    listEl.innerHTML = Object.keys(BUTTON_SETTING_LABELS).map(key => {
      const meta = BUTTON_SETTING_LABELS[key];
      const checked = settings[key] ? 'checked' : '';
      return `
        <div class="notif-setting-row">
          <div>
          ...
          <label class="toggle-switch">
            <input type="checkbox" data-button-setting="${key}" ${checked}>
            <span class="toggle-switch__track"></span>
          </label>
        </div>
      `;
    ...
    listEl.querySelectorAll('[data-button-setting]').forEach(input => {
      input.addEventListener('change', () => {
        const key = input.dataset.buttonSetting;
        const result = PO_Content.updateButtonSetting(key, input.checked);
        if (!result.ok) {
          input.checked = !input.checked;
          showToast(result.error || "Impossible d'enregistrer ce réglage.", 'error');
          return;
        }
        showToast(
          input.checked ? `Bouton « ${BUTTON_SETTING_LABELS[key].label} » activé.` : `Bouton « ${BUTTON_SETTING_LABELS[key].label} » désactivé.`,
          'success'
        );
      });
    });
  ...
  async function renderTestimonialsAdmin() {
    ...
    await PO_Testimonials.refreshAdmin();
    ...
    tbody.querySelectorAll('[data-testi-approve]').forEach(btn => {
      btn.addEventListener('click', async () => {
        await PO_Testimonials.adminApprove(btn.dataset.testiApprove);
    ...
    tbody.querySelectorAll('[data-testi-reject]').forEach(btn => {
      btn.addEventListener('click', async () => {
        await PO_Testimonials.adminReject(btn.dataset.testiReject);
    ...
    tbody.querySelectorAll('[data-testi-delete]').forEach(btn => {
      btn.addEventListener('click', () => {
        askConfirm('Supprimer ce témoignage ?', 'Cette action est irréversible.', async () => {
          await PO_Testimonials.adminDelete(btn.dataset.testiDelete);
          renderTestimonialsAdmin();
        });
      });
    });
    tbody.querySelectorAll('[data-testi-edit]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        await PO_Testimonials.adminUpdate(t.id, { text: newText });
  ...
  document.addEventListener('po:testimonials-updated', async () => {
    await PO_Testimonials.refreshAdmin();
    updateTestimonialsBadge();
  ...
  updateMessengerBadge();
  updatePaymentsBadge();
  if (typeof PO_Testimonials !== 'undefined') {
    PO_Testimonials.refreshAdmin().then(() => updateTestimonialsBadge());
  }
});

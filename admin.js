...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-calsub]').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('[data-calsub]').forEach(b => b.setAttribute('aria-selected', String(b === btn)));
  ...
  function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const chips = dayAppts.slice(0, 3).map(a => {
        ...
        return `<span class="day-appt-chip day-appt-chip--${tone}" draggable="${a.blocked ? 'false' : 'true'}" data-chip-appt="${a.id}" title="${escapeHtml(label)}">${escapeHtml(label)}</span>`;
      }).join('');
    ...
    list.innerHTML = appts.map(a => `
      <div class="day-appt-row">
        ...
        ${a.blocked
          ...
          : `<span class="badge badge--${a.status}">${statusLabel(a.status)}</span>`}
        <div class="admin-row-actions">
          ${a.blocked ? '' : `<button class="icon-btn" data-edit-appt="${a.id}" title="Modifier">✎</button>`}
          ${a.recurringGroupId
            ? `<button class="icon-btn icon-btn--danger" data-delete-series="${a.recurringGroupId}" title="Supprimer toute la série">⌫</button>`
            : ''}
  ...
  document.getElementById('open-block-modal').addEventListener('click', () => {
    ...
    blockModalVeil.hidden = false;
  });
  document.getElementById('block-modal-cancel').addEventListener('click', () => { blockModalVeil.hidden = true; });
  ...
  document.getElementById('open-vacation-modal').addEventListener('click', () => {
    ...
    vacationModalVeil.hidden = false;
  });
  document.getElementById('vacation-modal-cancel').addEventListener('click', () => { vacationModalVeil.hidden = true; });
  ...
  function renderAvailabilityPanel() {
    ...
    grid.innerHTML = WEEKDAY_ORDER.map(dayKey => {
      ...
      const rangesHtml = ranges.map((r, i) => `
        <span class="weekly-hours-range" data-day="${dayKey}" data-index="${i}">
          <input type="time" value="${r.start}" data-range-field="start">
          <span>—</span>
          <input type="time" value="${r.end}" data-range-field="end">
          <button type="button" data-remove-range title="Retirer cette plage">✕</button>
        </span>`).join('');
      return `
    ...
    function collectAndSave(dayKey) {
      const row = grid.querySelector(`[data-day-row="${dayKey}"]`);
      const ranges = Array.from(row.querySelectorAll('.weekly-hours-range')).map(rangeEl => ({
    ...
    grid.querySelectorAll('[data-add-range]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const av2 = PO_Auth.getAvailability();
        const current = (av2.weeklyHours[serviceId] && av2.weeklyHours[serviceId][dayKey]) || [];
        const updated = [...current, { start: '09:00', end: '12:00' }];
        PO_Auth.updateWeeklyHours(serviceId, dayKey, updated);
        renderAvailabilityPanel();
      });
    });
  ...
  function renderVacationsList() {
    ...
    list.innerHTML = sorted.map(v => `
      <div class="vacation-row">
        ...
        <button class="icon-btn icon-btn--danger" data-remove-vacation="${v.id}" title="Supprimer">✕</button>
      </div>
    `).join('');
    list.querySelectorAll('[data-remove-vacation]').forEach(btn => {
      btn.addEventListener('click', () => {
        PO_Auth.removeVacation(btn.dataset.removeVacation);
        renderAvailabilityPanel();
        renderCalendar();
      });
    });
  ...
  document.getElementById('avail-slot-duration').addEventListener('change', (e) => {
    const serviceId = document.getElementById('avail-service-select').value;
    PO_Auth.setSlotDuration(serviceId, e.target.value);
  });

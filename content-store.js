...
const PO_Content = (() => {
  ...
  // N'amorce que les clés réellement absentes côté serveur — ne réinitialise
  // jamais du contenu déjà sauvegardé (migration douce). La persistance
  // réelle vers le serveur exige une session admin (route protégée) ; un
  // visiteur anonyme obtient quand même un rendu correct grâce aux valeurs
  // par défaut en mémoire, sans tenter une écriture vouée à échouer (403).
  function _seedMissingKeys() {
    ...
    if (!_read(SITE_CONTENT_KEY)) { const v = _defaultSiteContent(); if (canPersist) _write(SITE_CONTENT_KEY, v); else _contentCache[SITE_CONTENT_KEY] = v; }
    if (!_read(BIO_KEY)) { const v = _defaultBio(); if (canPersist) _write(BIO_KEY, v); else _contentCache[BIO_KEY] = v; }
  }

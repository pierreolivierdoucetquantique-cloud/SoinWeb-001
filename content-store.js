...
const PO_Content = (() => {
  ...
  async function _write(key, value) {
    ...
    if (!_contentLoaded) return { ok: true, deferred: true };
    const res = await PO_Api.put(`/api/admin/content/${encodeURIComponent(key)}`, { value });
    if (!res.ok) {
      console.warn(`[PO_Content] Échec de sauvegarde serveur pour "${key}" :`, res.error);
    } else if (typeof document !== 'undefined') {
      // Synchronisation temps réel : les composants déjà affichés sur la
      // page (ex. panneau d'admin ouvert dans un autre onglet du même
      // navigateur) peuvent écouter cet événement pour se rafraîchir
      // immédiatement, sans attendre un rechargement — même mécanisme que
      // "po:prices-updated" déjà utilisé pour les tarifs.
      document.dispatchEvent(new CustomEvent('po:content-updated', { bubbles: true, detail: { key } }));
    }
    return res;
  }
  ...
  function _defaultBio() {
    return {
      hero: {
        title: 'Je suis Pierre-Olivier',
        subtitle: 'Un chemin de reconnexion intérieure'
      },
      photo: '',
      sections: [
        {
          ...
          content: "Je suis Pierre-Olivier.\n\nJe n'aime pas me définir à travers des mots, des titres ou des étiquettes. Bien qu'ils puissent être utiles pour communiquer, ils deviennent parfois des limites que nous nous imposons inconsciemment. Lorsqu'on s'identifie uniquement à une fonction ou à une définition, il devient facile de se fermer à des possibilités, à des expériences et à une évolution bien plus vaste que ce que nous croyons être.\n\nJe préfère donc demeurer ouvert au mouvement de la vie et à ce qu'elle révèle continuellement.\n\nSi je devais malgré tout me décrire, je dirais simplement que je suis un être ayant consacré une grande partie de son existence à réunifier les différentes facettes de lui-même. Un chemin de reconnexion intérieure qui m'a permis d'intégrer différentes dimensions de mon être, tout en comprenant que l'évolution ne possède ni fin ni définition absolue.\n\nChaque étape ouvre la porte à une nouvelle compréhension de soi, du vivant et de la conscience."
        },
        {
          id: 'section_2',
          type: 'text',
          heading: 'Le commencement',
          content: "À l'âge de sept ans, je vivais déjà des expériences qui dépassaient tout ce que je pouvais comprendre.\n\nJ'entendais mon grand-père, alors décédé, prononcer mon prénom comme si sa présence demeurait encore à mes côtés.\n\nCes expériences ont profondément marqué mon enfance et sont devenues les premières pierres d'un long chemin intérieur."
        },
        {
          id: 'section_3',
          type: 'text',
          heading: 'Comprendre l\'humain',
          content: "Durant mes années au secondaire, une fascination grandissante pour l'être humain est apparue.\n\nJe me suis naturellement dirigé vers l'étude du langage non verbal ainsi que de la synergologie afin de mieux comprendre les émotions, les comportements, les mécanismes inconscients et les messages subtils que le corps exprime constamment.\n\nCette curiosité est rapidement devenue une véritable passion."
        },
        {
          id: 'section_4',
          type: 'text',
          heading: 'Le tournant',
          content: "À l'âge de dix-huit ans, mon initiation au premier degré Reiki a marqué un véritable point de bascule.\n\nCette expérience a réveillé en moi des capacités qui semblaient attendre le bon moment pour s'exprimer.\n\nÀ partir de cette période, mon évolution s'est accélérée naturellement.\n\nLes années suivantes ont été consacrées à approfondir mes perceptions, explorer la conscience, comprendre l'énergie sous différentes formes, expérimenter et apprendre auprès de nombreuses expériences de vie."
        },
        {
          id: 'section_5',
          type: 'text',
          heading: 'La grande transformation',
          content: "Aujourd'hui, à bientôt trente-quatre ans, je peux dire que ma plus grande école aura été la vie elle-même.\n\nJ'ai traversé une profonde restructuration intérieure.\n\nCe que plusieurs appellent la « nuit noire de l'âme » est venue frapper à ma porte.\n\nElle a remis en question mes certitudes, mes repères, mes croyances et parfois même mon identité.\n\nAvec le recul, je comprends que cette période n'était pas une destruction.\n\nElle était une reconstruction.\n\nLa vie a placé sur mon chemin exactement les personnes que je devais rencontrer afin de retrouver mon propre pouvoir, ma souveraineté personnelle et une compréhension beaucoup plus profonde de qui je suis."
        },
        {
          id: 'section_6',
          type: 'text',
          heading: 'Ma vision',
          content: "Je ne prétends pas détenir la vérité.\n\nJe ne souhaite convaincre personne.\n\nJe partage simplement mon expérience, mes apprentissages et ce que la vie m'a permis d'intégrer jusqu'à aujourd'hui.\n\nSi certaines personnes se reconnaissent dans ce parcours, alors peut-être que nos chemins avaient simplement rendez-vous."
        },
        {
          id: 'section_7',
          type: 'values',
          heading: 'Mes valeurs',
          content: "Authenticité · Bienveillance · Présence · Respect du rythme de chacun · Ancrage dans le corps · Ouverture à l'invisible · Écoute · Entraide · Détermination · Résilience · Humilité · Respect · Conscience · Évolution"
        },
        {
          id: 'section_8',
          type: 'text',
          heading: 'Ntabou Aka Wé',
          content: "Ntabou Aka Wé signifie :\n\n« Solide comme un rocher, guidé par une main puissante. »\n\nCe nom représente parfaitement la vision qui accompagne ce projet.\n\nLa stabilité intérieure.\nLa confiance.\nLa guidance.\nLa force tranquille.\n\nEt la capacité de continuer d'avancer, peu importe les tempêtes."
        },
        {
          id: 'section_9',
          type: 'text',
          heading: 'Ce site',
          content: "Ce site n'est pas simplement une plateforme.\n\nIl représente le résumé vivant de mes trente-trois premières années d'existence.\n\nChaque image…\nChaque symbole…\nChaque création…\nChaque service…\nChaque texte…\n\nChaque espace présent ici raconte une partie de mon histoire.\n\nIl est l'expression de mon chemin, de mes apprentissages, de mes remises en question et des nombreuses transformations qui m'ont façonné.\n\nJ'espère que vous y trouverez non pas des réponses toutes faites, mais peut-être une réflexion, une inspiration ou simplement un espace où vous pourrez avancer à votre propre rythme."
        },
        {
          id: 'section_10',
          type: 'quote',
          heading: '',
          content: "Même lorsque tu crois ne plus être capable d'avancer, il existe au plus profond de toi une étincelle qui continue de murmurer que tout ira bien. Fais-lui confiance, car c'est souvent elle qui éclaire le chemin lorsque tout semble plongé dans l'obscurité."
        },
        {
          id: 'section_11',
          type: 'quote',
          heading: '',
          content: "Se souvenir de qui nous sommes ne consiste pas seulement à se rappeler ce qui a précédé notre incarnation. C'est avant tout accueillir pleinement l'être que nous sommes devenu, en honorant chacune de nos expériences depuis le tout début de notre existence. Chaque étape, chaque apprentissage et chaque transformation participent à la mémoire vivante de notre être."
        }
      ]
    };
  }
  ...
  function _seedMissingKeys() {
    ...
    Object.keys(defaults.servicePages).forEach(serviceId => {
      const defaultPage = defaults.servicePages[serviceId];
      if (!content.servicePages[serviceId]) content.servicePages[serviceId] = {};
      const page = content.servicePages[serviceId];
      // Chaque champ est vérifié INDÉPENDAMMENT — pour qu'aucun texte ne
      // puisse rester vide même si le reste de la page a déjà été
      // personnalisé (ex. l'intro a été modifiée mais la FAQ, elle,
      // n'a jamais existé).
      if (!page.intro) { page.intro = defaultPage.intro; changed = true; }
      if (!Array.isArray(page.steps) || page.steps.length === 0) { page.steps = defaultPage.steps; changed = true; }
      if (!Array.isArray(page.faq) || page.faq.length === 0) { page.faq = defaultPage.faq; changed = true; }
      if (!page.ctaTitle) { page.ctaTitle = defaultPage.ctaTitle; changed = true; }
      if (!page.ctaText) { page.ctaText = defaultPage.ctaText; changed = true; }
      if (!page.ctaButtonLabel) { page.ctaButtonLabel = defaultPage.ctaButtonLabel; changed = true; }
    });
    if (changed) {
  ...
  async function updateService(id, patch) {
    ...
    const res = await _write(SERVICES_KEY, services);
  ...
  async function updateThreshold(patch) {
    const content = getSiteContent();
    content.threshold = { ...content.threshold, ...patch };
    const res = await _write(SITE_CONTENT_KEY, content);
    if (!res.ok) return { ok: false, error: res.error };
    return { ok: true, threshold: content.threshold };
  }

  async function updateServicesIntro(patch) {
    const content = getSiteContent();
  ...
  async function updateServiceCard(serviceId, patch) {
    const content = getSiteContent();
  ...
  async function updateSharedLabels(patch) {
    const content = getSiteContent();
  ...
  async function updateServicePageContent(serviceId, patch) {
    const content = getSiteContent();
    if (!content.servicePages) content.servicePages = {};
  ...
  async function updateServiceStep(serviceId, index, patch) {
    const content = getSiteContent();
  ...
  async function updateServiceFaq(serviceId, index, patch) {
    const content = getSiteContent();
    if (!content.servicePages || !content.servicePages[serviceId]) {
      return { ok: false, error: 'Page de service introuvable.' };
    }
  ...
  async function addStep(serviceId, step) {
    const content = getSiteContent();
  ...
  async function removeStep(serviceId, index) {
    const content = getSiteContent();
    const page = _getServicePageOrFail(content, serviceId);
  ...
  async function moveStep(serviceId, index, direction) {
    const content = getSiteContent();
    const page = _getServicePageOrFail(content, serviceId);
  ...
  async function addFaqItem(serviceId, faq) {
    const content = getSiteContent();
    const page = _getServicePageOrFail(content, serviceId);
  ...
  async function removeFaqItem(serviceId, index) {
    const content = getSiteContent();
    const page = _getServicePageOrFail(content, serviceId);
  ...
  async function moveFaqItem(serviceId, index, direction) {
    const content = getSiteContent();
    const page = _getServicePageOrFail(content, serviceId);
    const faq = page.faq;
    const target = index + direction;
  ...
  async function updateBioHero(data) {
    const bio = getBio();
    bio.hero = Object.assign({}, bio.hero, data);
    const res = await _write(BIO_KEY, bio);
    if (!res.ok) return { ok: false, error: res.error };
    return { ok: true, hero: bio.hero };
  }

  // Remplace en une seule écriture atomique le titre/sous-titre et la
  // totalité des sections de la biographie (ex. chargement d'un nouveau
  // texte rédigé en une fois depuis l'admin). La photo n'est jamais touchée
  // ici : seul updateBioPhoto() peut la modifier, pour ne jamais écraser
  // une photo déjà mise en ligne par accident.
  async function replaceBioContent({ hero, sections } = {}) {
    const bio = getBio();
    if (hero) bio.hero = Object.assign({}, bio.hero, hero);
    if (Array.isArray(sections)) bio.sections = sections;
    const res = await _write(BIO_KEY, bio);
    if (!res.ok) return { ok: false, error: res.error };
    return { ok: true, hero: bio.hero, sections: bio.sections };
  }

  async function updateBioPhoto(photoDataUrl) {
    const bio = getBio();
  ...
  async function addBioSection(type) {
    const bio = getBio();
  ...
  async function updateBioSection(id, data) {
    const bio = getBio();
  ...
  async function removeBioSection(id) {
    const bio = getBio();
  ...
  async function moveBioSection(id, direction) {
    const bio = getBio();
  ...
  return {
    ...
    getBio, getDefaultBio: _defaultBio, updateBioHero, replaceBioContent, updateBioPhoto, addBioSection, updateBioSection, removeBioSection, moveBioSection,
    // Email templates

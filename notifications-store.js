...
const PO_Notifications = (() => {
  ...
  const EMAIL_TYPES = {
    ...
    appointment_awaiting_confirmation: 'Replanification en attente de confirmation',
  ...
  function _seed() {
    if (_read(SETTINGS_KEY, null) === null) {
      _write(SETTINGS_KEY, {
        ...
        appointment_awaiting_confirmation: true,
        admin_message: true,
        password_reset: true,
    ...
    // Migration douce : si des réglages existaient déjà (avant l'ajout des
    // nouveaux types de statut), on les active par défaut sans toucher au reste.
    const existing = _read(SETTINGS_KEY, null);
    if (existing) {
      let changed = false;
      ['care_session_summary', 'appointment_declined', 'appointment_rescheduled', 'appointment_completed', 'appointment_awaiting_confirmation'].forEach(key => {
        if (existing[key] === undefined) { existing[key] = true; changed = true; }
      });
      if (changed) _write(SETTINGS_KEY, existing);
    }
  }
  _seed();

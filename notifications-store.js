...
const PO_Notifications = (() => {
  ...
  function _seed() {
    ...
    // Migration douce : activer les nouveaux types sur les instances existantes
    const existing = _read(SETTINGS_KEY, null);
    if (existing) {
      let changed = false;
      ['appointment_declined', 'appointment_rescheduled',
       'appointment_completed', 'appointment_awaiting_confirmation',
       'payment_received', 'payment_confirmed', 'payment_refused'].forEach(key => {

...
//
// Le paiement par carte est réellement traité par Stripe (test ou réel selon
// les clés configurées sur le serveur — voir PO_StripePayment.isTestMode()) via
...
const PO_Auth = (() => {
  ...
  const PASSWORD_RESETS_KEY = 'po_password_resets';
  const RESET_TOKEN_TTL_MS = 60 * 60 * 1000; // 1 heure

  function _readResets() {
    try { return JSON.parse(localStorage.getItem(PASSWORD_RESETS_KEY)) || []; }
    catch { return []; }
  }
  function _writeResets(list) {
    localStorage.setItem(PASSWORD_RESETS_KEY, JSON.stringify(list));
  }
  ...
  /**
   * Demande de réinitialisation : génère un jeton à usage unique, valide 1h.
   * NOTE D'ARCHITECTURE : ce jeton est stocké dans le localStorage de CE
   * navigateur. Le lien envoyé par courriel ne fonctionnera donc que si la
   * personne l'ouvre depuis le même navigateur/appareil qui a fait la
   * demande (le cas le plus fréquent : elle consulte son courriel sur le
   * même téléphone/ordinateur). Une vraie validation multi-appareils
   * nécessite la migration Supabase (le jeton devrait alors être vérifié
   * côté serveur, pas dans localStorage). Voir PLAN-MIGRATION-SUPABASE.md.
   * @returns {{ok: boolean, exists: boolean, token?: string}}
   */
  function requestPasswordReset(email) {
    ...
    const resets = _readResets().filter(r => r.email.toLowerCase() !== email.toLowerCase());
    const token = _genResetToken();
    resets.push({ email: user.email, token, expiresAt: Date.now() + RESET_TOKEN_TTL_MS, used: false });
    _writeResets(resets);
    return { ok: true, exists: true, token };
  }

  /**
   * Vérifie qu'un jeton de réinitialisation est valide (existe, correspond
   * à l'email, n'a pas expiré, n'a pas déjà été utilisé).
   */
  function verifyResetToken(email, token) {
    const reset = _readResets().find(r =>
      r.email.toLowerCase() === (email || '').toLowerCase() && r.token === token
    );
    if (!reset) return { ok: false, error: 'Lien de réinitialisation invalide.' };
    if (reset.used) return { ok: false, error: 'Ce lien a déjà été utilisé.' };
    if (Date.now() > reset.expiresAt) return { ok: false, error: 'Ce lien a expiré. Merci de refaire une demande.' };
    return { ok: true };
  }

  /**
   * Définit un nouveau mot de passe à partir d'un jeton valide, puis
   * invalide le jeton (usage unique).
   */
  function resetPasswordWithToken(email, token, newPassword) {
    const check = verifyResetToken(email, token);
    ...
    users[idx].password = newPassword; // NOTE: démonstration — un vrai backend doit hasher ce champ
    _writeUsers(users);

    const resets = _readResets().map(r =>
      (r.email.toLowerCase() === email.toLowerCase() && r.token === token) ? { ...r, used: true } : r
    );
    _writeResets(resets);
    return { ok: true };
  }
  ...
  return {
    ...
    updateProfile, deleteAccount, requestPasswordReset, verifyResetToken, resetPasswordWithToken,

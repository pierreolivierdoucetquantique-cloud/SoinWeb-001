...
const PO_Auth = (() => {
  ...
  // ---- Compte / session ----
  async function createAccount({ firstName, lastName, email, age, password, privacyConsent }) {
    const res = await PO_Api.post('/api/auth/register', { firstName, lastName, email, age, password, privacyConsent });
  ...
  async function createRecurringAppointments({ clientId, clientName, service, serviceId, date, time, duration, status, frequency, occurrences }) {
    ...
    for (let i = 0; i < (occurrences || 1); i++) {
      ...
      else skipped.push({ date: dateStr, time, reason: res.error || 'Créneau déjà occupé.' });

...
function loadPage(file, url) {
  ...
  dom.window.fetch = async (url, opts) => {
    const realUrl = String(url).replace('https://soinweb-001.onrender.com', 'http://localhost:5097');
...
`);
  ...
  const ADMIN_FILES = ['api-client.js', 'auth.js', 'profile-photo.js', 'content-store.js', 'pricing.js',
    'email-service.js', 'messenger-store.js', 'notifications-store.js', 'care-session-store.js',
...
`);
  ...
  await evalScripts(domProfil, [...COMMON, 'notifications-store.js', 'messenger-store.js', 'care-session-store.js', 'email-service.js', 'profile-photo.js'], '\n;\n' + profilInline);
  await new Promise(r => setTimeout(r, 100));

...
async function confirmAppointmentPayment(transactionId) {
  ...
  const adminSubject = `Rendez-vous confirmé — ${client.first_name} ${client.last_name} (${confirmationNumber || tx.id})`;
  const adminBody = [
    `Client : ${client.first_name} ${client.last_name} (${client.email})`,
    `Service : ${appt.service}`,
    `Statut du paiement : payé`,
    `Date : ${_dayOfWeekFr(appt.date)} ${appt.date} à ${appt.time}`,
    steps.zoom?.ok ? `Zoom : ${steps.zoom.joinUrl}` : 'Zoom : non créé',
    `Numéro de confirmation : ${confirmationNumber || '—'}`
  ].join('\n');
  _logNotification('admin_appointment_confirmed', ADMIN_EMAIL, adminSubject, adminBody);
  steps.adminNotification = { ok: true };
...
module.exports = { confirmAppointmentPayment, handleAppointmentCancelled, handleAppointmentRescheduled };

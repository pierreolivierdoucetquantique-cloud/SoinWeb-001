...
const { handleAppointmentCancelled, handleAppointmentRescheduled, notifyAdminNewBookingRequest } = require('./confirmation-workflow');
...
function registerAppointmentsRoutes(app) {
  ...
  app.post('/api/appointments', requireAuth, json, async (req, res) => {
    ...
    // Notification à l'administrateur — ne doit jamais faire échouer la
    // réponse au client si l'envoi de courriel échoue (déjà géré en interne
    // par notifyAdminNewBookingRequest, qui n'émet jamais d'exception).
    notifyAdminNewBookingRequest(id).catch(err => {
      console.warn('[Appointments] Notification admin (nouvelle demande) échouée:', err.message);
    });
  });
  ...
  app.put('/api/appointments/:id', requireAuth, json, async (req, res) => {
    ...
    // SÉCURITÉ : un client ne peut jamais s'auto-confirmer un rendez-vous —
    // "confirmed" ne peut provenir que du paiement réellement vérifié
    // (voir confirmation-workflow.js, déclenché par le webhook Stripe ou
    // la confirmation Interac par l'admin). Un client ne peut agir ici que
    // sur son propre rendez-vous, et uniquement pour l'annuler.
    const CLIENT_ALLOWED_STATUSES = ['cancelled'];
    if (req.user.role !== 'admin' && patch.status !== undefined && !CLIENT_ALLOWED_STATUSES.includes(patch.status)) {
      return res.status(403).json({ ok: false, error: 'Un client ne peut pas définir ce statut directement.' });
    }
    ...
    // Zoom/Google Calendar : propager l'annulation ou la replanification
    // d'un rendez-vous déjà confirmé (silencieux si rien n'a jamais été créé).
    try {
      if (patch.status === 'cancelled' && (appt.zoom_meeting_id || appt.google_event_id)) {
        await handleAppointmentCancelled(req.params.id);
      } else if (dateTimeChanged && appt.status === 'confirmed' && (appt.zoom_meeting_id || appt.google_event_id)) {
        await handleAppointmentRescheduled(req.params.id);
      }
    } catch (err) {
      console.warn('[Appointments] Synchronisation Zoom/Calendar échouée:', err.message);
    }

...
function registerAuthRoutes(app) {
  ...
  app.post('/api/auth/register', json, (req, res) => {
    const { firstName, lastName, email, age, password, privacyConsent } = req.body || {};
    if (!firstName || !lastName || !email || !password) {
      return res.status(400).json({ ok: false, error: 'Champs requis manquants.' });
    }
    if (!privacyConsent) {
      return res.status(400).json({ ok: false, error: 'Le consentement à la Politique de confidentialité est requis.' });
    }
    ...
    db.prepare(`
      INSERT INTO users (id, first_name, last_name, email, password_hash, age, role, privacy_consent_at)
      VALUES (?, ?, ?, ?, ?, ?, 'client', ?)
    `).run(id, firstName, lastName, email.toLowerCase(), hash, age || null, new Date().toISOString());

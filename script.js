...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  /* ---------- THRESHOLD MODAL ---------- */
  // Ce gate d'entrée ("Lieu Sacré") n'existe que sur la page d'accueil
  // (index.html). Sur toutes les autres pages, #threshold est absent du
  // DOM : le bloc entier est donc gardé par cette vérification pour éviter
  // un plantage (TypeError sur setAttribute) qui, avant ce correctif,
  // interrompait silencieusement TOUT le reste du script — y compris le
  // rendu des descriptions de service plus bas dans ce fichier — dès lors
  // que le seuil avait déjà été accepté une fois dans la session du
  // navigateur (sessionStorage conservé d'une page à l'autre).
    ...
    if (serviceId) {
      ...
      if (page) {
        ...
        if (stepsContainer && Array.isArray(page.steps)) {
          // Rune de la page (déjà affichée dans le hero) réutilisée comme
          // séparateur signature entre chaque section détaillée, pour une
          // identité visuelle cohérente sans dépendre d'un nouveau champ admin.
          const dividerGlyph = (service && service.rune) ? service.rune : '✦';
          const dividerHtml = `<span class="svc-divider" aria-hidden="true"><span class="svc-divider__line"></span><span class="svc-divider__glyph">${dividerGlyph}</span><span class="svc-divider__line"></span></span>`;
          const gridIcons = ['✦', '◈', '⟡', '✧', '☾', '✺'];

          stepsContainer.innerHTML = page.steps.map((s) => {
            const eyebrowHtml = s.eyebrow ? `<p class="svc-eyebrow">${escapeHtmlGlobal(s.eyebrow)}</p>` : '';
            const titleHtml = s.title ? `<h2 class="svc-heading">${escapeHtmlGlobal(s.title)}</h2>` : '';
            const introHtml = s.intro ? `<p class="svc-lead">${escapeHtmlGlobal(s.intro)}</p>` : '';
            const headHtml = `<div class="svc-block__head">${eyebrowHtml}${titleHtml}${dividerHtml}${introHtml}</div>`;

            let bodyHtml = '';
            if (s.type === 'list') {
              const items = String(s.text || '').split('\n').map(t => t.trim()).filter(Boolean);
              bodyHtml = `<div class="svc-grid">${items.map((t, i) => `
                <div class="svc-grid__card">
                  <span class="svc-grid__icon" aria-hidden="true">${gridIcons[i % gridIcons.length]}</span>
                  <p>${escapeHtmlGlobal(t)}</p>
                </div>
              `).join('')}</div>`;
            } else {
              const paragraphs = String(s.text || '').split(/\n\s*\n/).map(p => p.trim()).filter(Boolean);
              bodyHtml = `<div class="svc-card"><div class="se-prose">${paragraphs.map(p => `<p>${escapeHtmlGlobal(p)}</p>`).join('')}</div></div>`;
            }
            const outroHtml = s.outro ? `<p class="svc-outro">${escapeHtmlGlobal(s.outro)}</p>` : '';
            return `<section class="page-section svc-block">${headHtml}${bodyHtml}${outroHtml}</section>`;
          }).join('');
        }

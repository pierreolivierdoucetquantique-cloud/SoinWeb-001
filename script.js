...
document.addEventListener('DOMContentLoaded', () => {
      ...
      formulasContainer.innerHTML = formulas.map(f => `
        <div class="formula-card ${f.featured ? 'formula-card--featured' : ''}">
          ...
          ${serviceId === 'soins-direct'
            ? `<button type="button" class="btn btn--accent bf-formula-btn" data-formula-id="${escapeHtmlGlobal(f.id)}" data-formula-title="${escapeHtmlGlobal(f.title)}" data-formula-price="${f.price}" data-formula-duration="${escapeHtmlGlobal(f.duration)}">Réserver</button>`
  ...
  (function initTestimonials() {
    ...
    form?.addEventListener('submit', (e) => {
      ...
      const result = PO_Testimonials.submit({
        clientId: user.id,
